<template>
  ...
  <div v-else class="min-h-screen bg-farm-bg text-zinc-800 lg:grid lg:grid-cols-[175px_minmax(0,1fr)]">
    ...
    <aside class="hidden flex-col border-r-2 border-black/10 bg-gradient-to-r from-[#2f5736] to-[#355f3a] text-[#f4efe4] lg:flex">
      <div class="flex items-center gap-3 border-b border-white/10 px-4 py-4">
        <img src="/assets/logo.png" alt="FarmPro" class="h-10 w-10 rounded-lg object-cover" />
        <div>
          <h1 class="m-0 text-2xl font-bold leading-none">FarmPro</h1>
          <p class="mt-1 text-xs text-[#d7d1c2]">Livestock Manager</p>
        </div>
      </div>
      ...
      <nav class="flex flex-col gap-1 px-2 py-3">
        ...
        >
          <component :is="navIconMap[item.icon]" class="h-4 w-4" />
          {{ item.name }}
        </RouterLink>
      </nav>

      <div class="mt-auto flex items-center gap-3 border-t border-white/10 px-4 py-3">
        <div class="grid h-9 w-9 place-items-center rounded-full bg-[#4c7152] text-xs font-bold">{{ initials }}</div>
        <div>
      ...
      <button
        class="mx-4 mb-4 rounded-lg border border-white/35 bg-transparent px-3 py-2 text-xs text-[#f5f1e6]"
        @click="logout"
      >
        Logout
      </button>
    ...
    <Teleport to="body">
      <div v-if="mobileNavOpen" class="fixed inset-0 z-50 lg:hidden">
        ...
        <aside class="relative h-full w-[78%] max-w-[260px] bg-gradient-to-r from-[#2f5736] to-[#355f3a] text-[#f4efe4]">
          <div class="flex items-center justify-between border-b border-white/10 p-4">
            <div class="flex items-center gap-2">
              <img src="/assets/logo.png" alt="FarmPro" class="h-8 w-8 rounded-md object-cover" />
              <strong class="text-lg">FarmPro</strong>
            </div>
